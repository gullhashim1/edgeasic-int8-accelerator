...
module dma_read_router (
    ...
    input  logic [511:0] axi_rdata,      // 512-bit (64-byte) beat from memory[cite: 1, 8]

    // Target Selection from Prefetch FSM
    input  logic [1:0]  target_sram_sel, // 00=Act, 01=Wgt, 10=Bias/Quant, 11=Res[cite: 8]
    input  logic        target_bank_idx, // Ping-pong bank 0 or 1[cite: 8]
    ...
    // Direct byte-lane mapping from AXI beat to internal SRAM word[cite: 8]
